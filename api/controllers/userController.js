...
exports.createUser = (req, res) => {
  ...
  // 4- Check if phone is already taken
  // 5- encrypt the password and save the new user
  ...
  // 4- Check if the phone is taken
  function checkIfPhoneIsTaken() {
    User.findOne({ phone: req.body.phone }, (err, userWithSamePhone) => {
      if (err) {
        res.status(400).json({
          message: 'Error getting phone try gain',
        });
      } else if (userWithSamePhone) {
        res.status(400).json({ message: 'This phone number is taken' });
      } else {
        encryptDataAndSave();
      }
    });
  }
  // 5- encrypt the password using bcryptjs and save the new user with hashed pass
...
// handle POST request at "api/users/loginUser"
  ...
  User.findById(req.user.id, (err, userToUpdate) => {
    if (err) {
      ...
      let updatedUser = {
        ...
        location: req.body.location ? req.body.location : userToUpdate.location,
      ...
      if (userToUpdate.phone === req.body.phone) {
        User.findOne({ phone: req.body.phone }, (err, userWithSamePhone) => {
          if (err) {
            res.status(400).json({
              message: 'Error getting username',
